...
const NavItem = props => {
  ...
  return (
    ...
    {props.url != null ? 
     ...
     to = {props.url} activeClassName = "active"> 
      <div ref={div => (circle = div)} className="circle"></div>
      <div className="circle circle2"></div>
      <span>{props.name} </span> 
      </TransitionLink>
          :
    <a href = {props.storeLink} target = "__blank">
        <div ref={div => (circle = div)} className="circle"></div>
      <div className="circle circle2"></div>
      <span>{props.name} </span> 
    </a>
...
const LargeScreen = props => {
  ...
  const handleClick = useCallback(() => {
    ...
    tl2.staggerTo(backItems, duration,{
        opacity: 0,
        x: toX,
        ease: Ease,
    }, stagger)    
    tl2.staggerTo(socialItems, duration,{
        opacity: 0,
        x: toX,
        ease: Ease,
    }, stagger, 0)    
    tl2.staggerTo(navItems, duration,{
        opacity: 0,
        x: toX,
        ease: Ease,
    }, stagger, `-=${duration * .8}`)    
    tl2.to(image, duration,{
        opacity: 0,
        x: toX,
        ease: Ease,
    }, stagger, `-=${duration}`)
    tl2.staggerTo(rows, duration ,{
        opacity: 0,
        width: "0%",
        ease: Ease,
    }, stagger, `-=${duration * .8}`)    
    ...
    <div className="largeScreen">
      ...
      <div className="grid">
        ...
        >
          ...
          url = {"/"} name="Home" />
          <NavItem transitionOut = {props.transitionOut} url = {"about"} name="About" />
          <NavItem transitionOut = {props.transitionOut} url = "collections" name="Collections" />
        ...
        <div className="social">
          <div ref={div => (social = div)}>
            <Social
              url={info.instagramLink}
              src={insta}
              alt="instagram"
            />
            <Social
              url={info.facebookLink}
              src={facebook}
              alt="facebook"
            />
            <Social
              url={info.pinterestLink}
              src={pintrest}
              alt="pinterest"
            />
          </div>
        ...
        <div ref={div => (image = div)} className="img">
          <ImgWindow image = {info.navPhoto.publicURL} />
        </div>
      </div>
    </div>
  )
